...
void Indie::Player::update(float delta)
{
    ...
    if (raylib::KeyboardHelper::isKeyDown(KEY_RIGHT) || raylib::GamepadHelper::isGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_RIGHT)) {
    ...
    if (raylib::KeyboardHelper::isKeyDown(KEY_LEFT) || raylib::GamepadHelper::isGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_LEFT)) {
        Vector3f newPosition =   {this->_position.x - speed, this->_position.y, this->_position.z};
        this->setRotationDegrees(0, {0, 1, 0});
        this->moveAndCollide(newPosition);
    }
    if (raylib::KeyboardHelper::isKeyDown(KEY_UP) || raylib::GamepadHelper::isGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_UP)) {
        Vector3f newPosition =   {this->_position.x, this->_position.y, this->_position.z - speed};
        this->setRotationDegrees(270, {0, 1, 0});
        this->moveAndCollide(newPosition);
    }
    if (raylib::KeyboardHelper::isKeyDown(KEY_DOWN) || raylib::GamepadHelper::isGamepadButtonDown(0, GAMEPAD_BUTTON_LEFT_FACE_DOWN)) {
        Vector3f newPosition =   {this->_position.x, this->_position.y, this->_position.z + speed};
    ...
    if (raylib::KeyboardHelper::isKeyPressed(KEY_SPACE) || raylib::GamepadHelper::isGamepadButtonDown(0, GAMEPAD_BUTTON_RIGHT_FACE_DOWN)) {
